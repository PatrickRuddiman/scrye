...
## Tasks
- [ ] In `crates/scryd-imap/src/fetch.rs`, replace the deferred `Err("…")` return body with the real implementation:
...
- [ ] Add an integration test `crates/scryd-imap/tests/fetch_live.rs`:
...
- [ ] Add a unit test for the missing-attr path: feed a hand-rolled `Fetch` with no UID and assert `ClientError::Server`. (Trickier; if `async-imap` doesn't expose a constructor for `Fetch` outside its own crate, mock at the function boundary by extracting the parsing into a helper `parse_fetch_row(fetch: &Fetch) -> Result<FetchedMessage, ClientError>` that's testable with a stub.)

## Acceptance criteria
- [ ] `cargo test -p scryd-imap --test fetch_live` passes locally with GreenMail running.
- [ ] `cargo test -p scryd-imap` overall passes (existing tests still green).
- [ ] `! git grep -F 'live response parsing is authored in a follow-up' crates/scryd-imap/src/fetch.rs` (the deferred-comment is gone).
- [ ] `git grep -nE 'pub async fn fetch_batch' crates/scryd-imap/src/fetch.rs` matches and the body no longer returns `Err`.
