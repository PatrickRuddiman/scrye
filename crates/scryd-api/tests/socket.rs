...
async fn bind_creates_socket_at_mode_0660() {
    ...
    let _listener = bind(&scryd_dir).await.expect("bind succeeds");
...
async fn bind_creates_parent_dir_at_mode_0700() {
    ...
    let _listener = bind(&scryd_dir).await.unwrap();
...
async fn second_bind_against_live_socket_returns_already_running() {
    ...
    let _first = bind(&scryd_dir).await.unwrap();
...
async fn stale_socket_left_by_a_dead_daemon_is_cleaned_up() {
    ...
    let _listener = bind(&scryd_dir).await.expect("rebind after stale file");
    assert!(sock.exists());
}
