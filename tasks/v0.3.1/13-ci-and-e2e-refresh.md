...
- [x] Update `tests/e2e_imap_to_search.sh`:
...
- [x] Update `.github/workflows/release.yml` to mirror the same cache step before the e2e leg.
- [x] Refresh `crates/scryd-api/tests/peercred.rs` and `peercred_accept_path.rs`: most tests already pass `enabled = true` per task 01; ensure the test names + assertions reflect that peercred is now opt-in (e.g., add a class-comment naming the v0.3.1 default).
...
- [x] `grep -F 'actions/cache' .github/workflows/ci.yml` matches the weights cache step.
- [x] `grep -F 'witchcraft.sqlite' tests/e2e_imap_to_search.sh` matches OR the persistence leg's restart-and-search assertion is otherwise visible.
- [x] `! grep -F '--remove-v01-data' tests/install_sh.sh`.
- [x] `grep -F '--accounts secondary' tests/e2e_imap_to_search.sh` matches the multi-account filter assertion.
