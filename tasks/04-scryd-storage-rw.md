...
- [x] Write integration tests in `crates/scryd-storage/tests/reconcile.rs`: load a config with two accounts, reconcile, assert two rows; reload with one removed, reconcile, assert one is `active=0`, the other unchanged; assert no password was persisted (column does not exist; `git grep` of the source confirms).

## Acceptance criteria
- [x] `cargo test -p scryd-storage` passes (all integration tests above + Task 03's tests).
- [x] `cargo check -p scryd-storage` exits 0.
- [x] `git grep -nE 'password' crates/scryd-storage/src/migrations/` returns no matches (confirms no `password` column anywhere in storage migrations).
- [x] `git grep -nE 'INSERT.*ON CONFLICT.*DO UPDATE' crates/scryd-storage/src/messages.rs` matches the idempotent insert.
