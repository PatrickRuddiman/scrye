...
## Acceptance criteria
- [ ] `cargo test -p scryd-imap --test tombstone_live` passes locally with GreenMail.
- [ ] `git grep -nE 'pub async fn scan' crates/scryd-imap/src/tombstone.rs` matches.
- [ ] Existing `cargo test -p scryd-imap --test tombstone` still passes (the `compute_tombstones` unit tests).
