...
- [x] Rewrite `ops/README.install.md`:
...
- [x] Create `tasks/v0.3.1/triage/` with seven short markdown notes (one per item from the plan's "Out of scope (triage docs only)" list). Each is 5-15 lines: title, why deferred, what triggers picking it up.

## Acceptance criteria
- [x] `test -f docs/security.md`.
- [x] `test -f scryd-spec.md && ! test -f scryd-spec-v0.2.0.md`.
- [x] `! grep -F 'operator' README.md | grep -iE 'uid|user' | head -1` returns no match (or only matches in deliberate "the operator runs sudo …" senses; manual review the few matches before checking the box).
- [x] `! grep -F 'v0.1.0 → v0.2.0' ops/README.install.md`.
- [x] `grep -F 'account_ids' README.md` matches.
- [x] `grep -F 'scryd sync' README.md` matches.
- [x] `grep -F 'scryd status' README.md` matches.
- [x] `grep -F 'docs/security.md' README.md` matches.
- [x] `ls tasks/v0.3.1/triage/*.md | wc -l` returns at least 7.
