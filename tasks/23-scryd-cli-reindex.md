...
## Tasks
- [ ] In `scryd/src/cmd_reindex.rs`, define a no-arg clap subcommand. Implement `pub async fn run() -> Result<(), CliError>`.
- [ ] Build a `UdsClient`. POST to `/internal/reindex` with no body. Map responses:
...
- [ ] Wire into the dispatcher in `scryd/src/main.rs`.
- [ ] Write integration tests in `scryd/tests/cmd_reindex.rs` using a stub api on a temp UDS:
...
## Acceptance criteria
- [ ] `cargo test -p scryd --test cmd_reindex` passes.
- [ ] `cargo build --release -p scryd` exits 0.
- [ ] `target/release/scryd reindex --help 2>&1 | grep -E 'Usage:.*reindex'` matches (no flags expected; just the verb).
- [ ] `git grep -nE 'StatusCode::ACCEPTED|202' scryd/src/cmd_reindex.rs` matches.
- [ ] `git grep -nE 'StatusCode::CONFLICT|409' scryd/src/cmd_reindex.rs` matches.
